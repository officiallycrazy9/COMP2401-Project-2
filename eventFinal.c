...
void event_queue_init(EventQueue *queue) {
	if (!queue) return;
    queue->head = NULL;//initialize to empty
    queue->size = 0;
}
...
void event_queue_clean(EventQueue *queue) {
	if (!queue) return;//check null

    EventNode *current = queue->head;//go through all nodes in queue
    EventNode *next = NULL;

    while (current) {//free each node one by one
        next = current->next;
        free(current);
        current = next;
    }
    queue->head = NULL;//reset to empty queue
    queue->size = 0;
}
...
void event_queue_push(EventQueue *queue, const Event *event) {
	if (!queue || !event) return;//check if null

    EventNode *new_node = malloc(sizeof(EventNode));//create a new node for event to be added
    if (!new_node) return;//exit if malloc fails

    new_node->event = *event;
    new_node->next = NULL;
	//check if queue is empty or new event has higher prioirity the the first event
    if (!queue->head || queue->head->event.priority < event->priority) {
        new_node->next = queue->head;
        queue->head = new_node;
    } else {
        EventNode *current = queue->head;//find correct position based on priority
        while (current->next && current->next->event.priority >= event->priority) {
            current = current->next;
        }
        new_node->next = current->next;//insert new node at approx position
...
int event_queue_pop(EventQueue *queue, Event *event) {
	if (!queue || !event || !queue->head) return 0;//check if queue or event pointer is NULL or if the queue is empty

    EventNode *to_remove = queue->head;//point to the first node to remove it
    *event = to_remove->event;//copy even from the node to the event pointer

    queue->head = to_remove->next;//move head of queue to next node
    free(to_remove);
    queue->size--;
