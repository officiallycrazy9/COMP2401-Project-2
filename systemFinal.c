...
void system_array_init(SystemArray *array) {
	if (!array) return;//Check if the array pointer is NULL

    array->systems = (System **)malloc(sizeof(System *));//allocate memory for an array of systm pointers (initially able to hold 1 system)
    if (!array->systems) return;//check if it failed
...
void system_array_clean(SystemArray *array) {
    ...
    for (int i = 0; i < array->size; ++i) {//loop through the systems and destroy them
        system_destroy(array->systems[i]);
    }

    free(array->systems);//free memory used for the array of systems then reset the array properties
    array->systems = NULL;
    array->size = 0;
    array->capacity = 0;
...
void system_array_add(SystemArray *array, System *system) {
	if (!array || !system) return;//check if array or system pointer is NULL

    if (array->size >= array->capacity) {//check for space in the array to add a system
    	int new_capacity = array->capacity * 2;//double capacity
        System **new_systems = (System **)malloc(new_capacity * sizeof(System *));//allocate a new larger array for them
        if (!new_systems) return;//check if it fails

        for (int i = 0; i < array->size; ++i) {//copy the existing sustems from the old array to the new one
            new_systems[i] = array->systems[i];
        }

        free(array->systems);//free the old array and point it to the new one
        array->systems = new_systems;
        array->capacity = new_capacity;
    }

    array->systems[array->size++] = system;//add the new syste to the array and increase size
